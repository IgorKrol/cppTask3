...
int main() {
  ...
  if (signal == 0) {
    ...
    // YOUR TESTS - INSERT AS MANY AS YOU WANT
      ...
      //distance:
      .CHECK_OUTPUT((dcm==dm), "1")
      .CHECK_OUTPUT((dm2==dkm), "1")
      .CHECK_OUTPUT((dkm==dm2), "1")
      .CHECK_OUTPUT((dm==dcm), "1")
      .CHECK_OUTPUT((dm2<dcm), "0")
      .CHECK_OUTPUT((dm!=dcm), "0")
      .CHECK_OUTPUT((dm>=dcm), "1")
      .CHECK_OUTPUT((dm<=dcm), "1")
      //time:
      .CHECK_OUTPUT((ts==tm), "1")
      .CHECK_OUTPUT((tm2==th), "1")
      .CHECK_OUTPUT((th==tm2), "1")
      .CHECK_OUTPUT((tm==ts), "1")
      //mass:
      .CHECK_OUTPUT((mg==mkg), "1")
      .CHECK_OUTPUT((mkg2==mt), "1")
      .CHECK_OUTPUT((mt==mkg2), "1")
      .CHECK_OUTPUT((mkg==mg), "1")

      .setname("Math calculations")
      //////// Distance /////////////
      .CHECK_OUTPUT((dcm+=dm), "200[cm]")
      .CHECK_OUTPUT(dcm, "200[cm]")
      .CHECK_OUTPUT(dkm+dcm, "1.002[km]")
      .CHECK_OUTPUT(dm+dm2, "1001[m]")
      .CHECK_OUTPUT((dcm+=dcm), "400[cm]")
      .CHECK_OUTPUT(dcm+km5, "500400[cm]")
      .CHECK_OUTPUT((dcm++), "400[cm]")
      .CHECK_OUTPUT(dcm, "401[cm]")
      .CHECK_OUTPUT((++dcm), "402[cm]")
      .CHECK_OUTPUT(dcm-m5, "-98[cm]")
      .CHECK_OUTPUT(m5-dcm, "0.98[m]")
      .CHECK_OUTPUT((dcm-=m5), "-98[cm]")
      ...
      .CHECK_OUTPUT(-(mt-t5), "4[ton]")
      .CHECK_OUTPUT((mkg2--), "1000[kg]")
      .CHECK_OUTPUT((--mkg2), "998[kg]")
